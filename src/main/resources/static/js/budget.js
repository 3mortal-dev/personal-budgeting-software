 ...
 *   id, user, category { id, name, ... }, spendingLimit,
...
function categoryMeta(name = "") {
  ...
  if (n.includes("food")  || n.includes("dining") || n.includes("restaurant"))
    return { color: "#10b981", bg: "#ecfdf5", icon: `<svg viewBox="0 0 24 24"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg>` };
  if (n.includes("transport") || n.includes("car")  || n.includes("fuel"))
    return { color: "#3b82f6", bg: "#eff6ff", icon: `<svg viewBox="0 0 24 24"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>` };
  if (n.includes("shop")  || n.includes("cloth")  || n.includes("fashion"))
    return { color: "#8b5cf6", bg: "#f5f3ff", icon: `<svg viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>` };
  if (n.includes("health") || n.includes("medical")|| n.includes("pharma"))
    return { color: "#ef4444", bg: "#fee2e2", icon: `<svg viewBox="0 0 24 24"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>` };
  if (n.includes("entertain")|| n.includes("fun")  || n.includes("leisure"))
    return { color: "#f59e0b", bg: "#fffbeb", icon: `<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8"/></svg>` };
  if (n.includes("util")  || n.includes("bill")   || n.includes("electric"))
    return { color: "#06b6d4", bg: "#ecfeff", icon: `<svg viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>` };
  if (n.includes("saving") || n.includes("invest") || n.includes("goal"))
    return { color: "#2aa96b", bg: "#e8f8f0", icon: `<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg>` };
  // Default fallback
  return { color: "#6b7280", bg: "#f3f4f6", icon: `<svg viewBox="0 0 24 24"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>` };
}
...
function openEditModal(id) {
  ...
  // Pre-select the category that matches the budget's category id
  const select = document.getElementById("budgetCategorySelect");
  if (select && b.category?.id) select.value = b.category.id;
...
function showError(msg) {
  ...
  if (el && text) { text.textContent = msg; el.classList.add("show"); }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (document.getElementById("budgetModal")?.classList.contains("is-open"))  { closeAddModal();      return; }
    if (document.getElementById("confirmModal")?.classList.contains("is-open")) { closeConfirmModal();  return; }
  ...
  document.querySelectorAll(".form-control").forEach(el => {
    el.addEventListener("input", () => { if (el.classList.contains("error")) clearFieldError(el); });
  });
});