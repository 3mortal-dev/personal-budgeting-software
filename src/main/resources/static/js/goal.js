...
function applyIcon(cls) {
  ...
  iconGrid
    ...
    .forEach((b) => b.classList.toggle("selected", b.dataset.icon === cls));
...
function selectColor(color) {
  ...
  colorSwatches
    ?.querySelectorAll(".color-swatch")
    .forEach((b) => b.classList.toggle("selected", b.dataset.color === color));
...
function renderNotifList() {
  ...
  notifListEl
    .querySelectorAll(".notif-item[data-id]")
    .forEach((el) =>
...
async function saveGoal(payload) {
  ...
  const res = await fetch(isEdit ? `${API_BASE}/${payload.id}` : API_BASE, {
...
async function updateGoalProgress(id, amount) {
  const res = await fetch(
    `${API_BASE}/${id}/progress?amount=${encodeURIComponent(amount)}`,
    {
      method: "PATCH",
      headers: csrfHeaders(),
    },
...
function buildGoalCard(goal) {
  ...
  card
    ...
    .addEventListener("click", () => openEditModal(goal.id));
  card
    .querySelector(".btn-icon--delete")
    .addEventListener("click", () => openDeleteModal(goal.id));
  card
    .querySelector(".btn-icon--progress")
...
function updateSummary() {
  ...
  if (targetAmountSummary)
    targetAmountSummary.textContent = formatMoney(target);
}
