...
function toastDashboardPartial() {
  showToast(
    "Some data is unavailable right now — showing what we have.",
    "warning",
  );
}
...
function toastBudgetAdded(categoryName, limit) {
  showToast(
    `Budget set for ${categoryName}: ${formatCurrency(limit)} / month`,
    "success",
  );
}
...
function toastBudgetWarning(categoryName, pct) {
  ...
  showToast(
    `${categoryName} budget is ${rounded}% used — you're close to the limit.`,
    "warning",
    4500,
  );
}
...
function toastBudgetExceeded(categoryName) {
  showToast(
    `${categoryName} budget exceeded! Consider adjusting your spending.`,
    "error",
    5000,
  );
}
...
function toastHighSpending(ratio) {
  ...
  showToast(
    `Heads up — you've used ${pct}% of this month's income.`,
    "warning",
    5000,
  );
}
...
function toastSpendingIncreased(increasePct) {
  showToast(
    `Spending is up ${Math.round(increasePct)}% vs. last month.`,
    "warning",
    4500,
  );
}
...
function toastNotificationsLoadFailed() {
  showToast(
    "Couldn't load notifications. They'll appear after a refresh.",
    "error",
  );
}
...
function renderDashboard() {
  ...
  setText(
    "statBudgetsSub",
    Number(data.activeBudgets || 0) === 1
      ? "1 active budget"
      : `${Number(data.activeBudgets || 0)} active budgets`,
  );
  setText("statGoals", Number(data.activeGoals || 0));
  setText(
    "statGoalsBadge",
    Number(data.activeGoals || 0) > 0
      ? `${Number(data.activeGoals)} active`
      : "",
  );
...
function renderMetricBars(monthlyIncome, monthlyExpense) {
  ...
  if (incomeBar)
    incomeBar.style.width = `${Math.min((income / max) * 100, 100)}%`;
  if (expenseBar)
    expenseBar.style.width = `${Math.min((expense / max) * 100, 100)}%`;
}
...
function renderNotificationList() {
  ...
  list.innerHTML = state.notifications
    .map((n) => {
      const rawType = (n.type || "INFO").toString().toUpperCase();
      const meta = NOTIF_TYPE_META[rawType] ?? {
        label: rawType.replace(/_/g, " "),
        icon: "🔔",
        cls: "notif--info",
      };
      const timeStr = n.createdAt ? formatDate(n.createdAt) : "";
      return `
    <div class="notif-item ${n.read ? "" : "unread"} ${meta.cls}" data-id="${n.id}" onclick="markRead(${n.id})">
      <div class="notif-item-icon" aria-hidden="true">${meta.icon}</div>
      <div class="notif-item-body">
        <div class="notif-item-title">${escapeHtml(meta.label)}</div>
...
function setTxFieldError(inputId, message) {
  ...
  if (!input) return;
  // Show error as a sibling span if it exists, else use native validation
...
function clearTxFormErrors() {
  document
    .querySelectorAll("#addTransactionForm .field-error")
    .forEach((el) => {
      el.textContent = "";
    });
}

function validateTransactionForm(type, amount, date, categoryId, source) {
  let valid = true;

  if (!amount || isNaN(amount) || amount <= 0) {
    setTxFieldError("transactionAmount", "Enter an amount greater than $0.");
    valid = false;
  }
  ...
  if (!date) {
    setTxFieldError(
      "transactionDate",
      "A date is required for this transaction.",
    );
    valid = false;
  } else {
    ...
    if (isNaN(picked.getTime()) || picked < hundredYearsAgo) {
      setTxFieldError("transactionDate", "Please enter a valid date.");
      valid = false;
    }
  }
...
async function addTransaction(event) {
  ...
  if (!validateTransactionForm(type, amount, date, categoryId, source)) {
    return;
  }
  ...
  if (result) {
    ...
    showToast(
      "Couldn't save the transaction. Check your connection and try again.",
      "error",
    );
  }
}
...
function clearBudgetFormErrors() {
  ...
  const banner = document.getElementById("budgetFormError");
  if (banner) banner.textContent = "";
}

function setBudgetFieldError(fieldErrId, message) {
...
function validateBudgetForm(categoryId, spendingLimit, threshold, startDate, endDate) {
  ...
  if (!categoryId || isNaN(categoryId)) {
    ...
    if (duplicate) {
      ...
      setBudgetFieldError(
        "budgetCategoryErr",
        `You already have an active budget for "${catName}". Delete it first or choose a different category.`,
      );
      valid = false;
    }
  }
  ...
  if (!endDate) {
    ...
    if (picked <= today) {
      setBudgetFieldError("budgetEndErr", "End date must be in the future.");
      valid = false;
    }
  }
...
async function addBudget() {
  ...
  if (!validateBudgetForm(categoryId, spendingLimit, threshold, startDate, endDate)) {
    ...
    return; // Don't start loading or hit the API
  }
  ...
  if (result) {
    ...
    showToast(
      "Couldn't create the budget — see the form for details.",
      "error",
    );
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.addEventListener("keydown", (event) => {
    ...
    closeAddTransactionModal();
    if (document.getElementById("addModal")?.classList.contains("is-open"))
      return closeAddModal();
    if (document.getElementById("budgetModal")?.classList.contains("is-open"))
      return closeBudgetModal();
    closeNotifications();
  });
